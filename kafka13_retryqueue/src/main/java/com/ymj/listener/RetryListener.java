...
import com.ymj.entity.RetryRecord;
...
public class RetryListener {
    ...
    public void consume(ConsumerRecord<String, String> record) {
        ...
        /**
         * 防止待重试消息太多撑爆redis，可以将待重试消息按下一次重试时间分开存放到不同介质
         * 例如下一次重试时间在半小时后的消息存储的mysql，并定时从mysql读取即将重试的消息存储到redis
         */
        ...
        redisTemplate.opsForZSet().add(RETRY_KEY_ZSET, key, retryRecord.getRetryTimes());
    }


    /**
     * 定时任务从redis中读取到达重试时间的消息，发送到对应的topic
     */
